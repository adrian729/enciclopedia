...
Backend that settles financial transactions between buyers, sellers, and intermediaries.
...
**Components** — Payment Service (orchestrator + risk check), Payment Executor (per-order execution), PSP, Card Schemes (Visa, MasterCard, Discover), Ledger, Wallet (merchant balance).
...
Mirror of pay-in: same component shape, but a third-party payable provider (e.g., **Tipalti**) moves e-commerce account → seller account. Trigger: fulfillment milestones (e.g., delivery confirmation) plus regulatory and bookkeeping checks before disbursement.
...
**`amount` is a string, not double** — per [Concurrency & Transactions §8](fundamentals/concurrency-and-transactions.md#8-double-entry-ledger), values can be huge (Japan's GDP ≈ 5×10¹⁴ yen) or tiny (1 satoshi = 10⁻⁸ BTC). Parse to numeric only at the display/calculation boundary.

**Double-entry ledger** — every transaction posts to two accounts with equal magnitude. Sum across all entries must equal 0. End-to-end traceability (Square's engineering blog cited).
...
1. **API integration** — the company stores sensitive card data itself (rare, expensive PCI compliance).
2. **Hosted page** — PSP captures and stores card data; the common choice.
...
- **Internal use** — also detects drift between internal services (e.g., ledger ↔ wallet).

**Mismatch buckets:**

1. Classifiable + auto-fixable (write a script).
2. Classifiable but not cost-effective to automate (finance team fixes from a job queue).
3. Unclassifiable (special queue, manual investigation).

## 7. Reliability: Retries and Failures

- **Append-only payment-state log** — persist every state transition so any failure can resume from the last known state.
- **Retry queue** — transient failures (network, timeouts) are routed here for automatic re-execution.
- **Dead letter queue** — messages exceeding the retry threshold; held for engineering investigation rather than silent loss. Uber's payment system uses Kafka for both. See [Queues & Messaging](fundamentals/queues-and-messaging.md) and [Resilience Patterns](fundamentals/resilience-patterns.md).
- **Non-retryable errors** — invalid input → stored in DB for review, not retried.
...
**Idempotency key** — client-generated UUID sent in HTTP header `<idempotency-key: key_value>`; for e-commerce, the **shopping-cart ID** just before checkout is a natural choice. Pattern recommended by Stripe and PayPal. The DB-level mechanism — using the key as a primary key with a unique constraint — is covered generically in [Concurrency & Transactions §7](fundamentals/concurrency-and-transactions.md#7-idempotency-keys-as-primary-keys).
...
- **Replica lag options** — read and write only from the primary (simple, replicas waste capacity), or keep replicas synchronously in agreement via Paxos/Raft, or use consensus-based stores like YugabyteDB or CockroachDB.
...
**Other operational topics** — monitoring (e.g., per-method acceptance rate), alerting on-call, debugging tools (transaction status, PSP records, processing history), currency exchange, region-specific payment methods, cash payment handling, Apple Pay / Google Pay integration.
