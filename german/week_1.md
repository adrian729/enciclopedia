# German Week 1: Foundations & Core A1 Grammar (A1)

## Table of Contents
- [1. German Pronunciation Guide](#1-german-pronunciation-guide)
  - [1.1. Vowels — Short & Long](#11-vowels-short-long)
  - [1.2. Consonants & Final Devoicing](#12-consonants-final-devoicing)
  - [1.3. Special Combinations](#13-special-combinations)
  - [1.4. Diphthongs](#14-diphthongs)
- [2. Essential Vocabulary (150 Words)](#2-essential-vocabulary-150-words)
  - [2.1. Nouns (40) — with Gender (der/die/das) & Plural](#21-nouns-40-with-gender-derdiedas-plural)
  - [2.2. Verbs (40)](#22-verbs-40)
  - [2.3. Adjectives (35)](#23-adjectives-35)
  - [2.4. Adverbs of Time & Frequency (15)](#24-adverbs-of-time-frequency-15)
  - [2.5. Adverbs of Place & Manner (10)](#25-adverbs-of-place-manner-10)
  - [2.6. Common Phrases & Particles (10)](#26-common-phrases-particles-10)
- [3. Numbers & Calendar](#3-numbers-calendar)
  - [3.1. Cardinal Numbers 0–100](#31-cardinal-numbers-0-100)
  - [3.2. Cardinal Numbers 100–1,000,000](#32-cardinal-numbers-100-1000000)
  - [3.3. Ordinal Numbers](#33-ordinal-numbers)
  - [3.4. Days of the Week](#34-days-of-the-week)
  - [3.5. Months](#35-months)
  - [3.6. Seasons](#36-seasons)
- [4. Personal Pronouns & Present Tense (Präsens)](#4-personal-pronouns-present-tense-präsens)
  - [4.1. Personal Pronouns](#41-personal-pronouns)
  - [4.2. Conjugation of sein](#42-conjugation-of-sein)
  - [4.3. Conjugation of haben](#43-conjugation-of-haben)
  - [4.4. Regular (Weak) Verb Pattern](#44-regular-weak-verb-pattern)
  - [4.5. Key Irregular Verbs in Präsens with Vowel Changes](#45-key-irregular-verbs-in-präsens-with-vowel-changes)
- [5. Nominative Case — The Subject](#5-nominative-case-the-subject)
  - [5.1. Definite Articles](#51-definite-articles)
  - [5.2. Indefinite Articles](#52-indefinite-articles)
  - [5.3. Identifying the Subject: "wer?" / "was?"](#53-identifying-the-subject-wer-was)
- [6. Accusative Case — Direct Objects](#6-accusative-case-direct-objects)
  - [6.1. Accusative Articles](#61-accusative-articles)
  - [6.2. Transitive Verbs That Take Accusative](#62-transitive-verbs-that-take-accusative)
  - [6.3. Recognizing Direct Objects: "wen?" / "was?"](#63-recognizing-direct-objects-wen-was)
- [7. Word Order — The V2 Rule & Questions](#7-word-order-the-v2-rule-questions)
  - [7.1. Verb-Second in Main Clauses](#71-verb-second-in-main-clauses)
  - [7.2. Subject-Verb Inversion When Position 1 ≠ Subject](#72-subject-verb-inversion-when-position-1-subject)
  - [7.3. Yes/No Questions — V1](#73-yesno-questions-v1)
  - [7.4. W-Questions](#74-w-questions)
- [8. Negation](#8-negation)
  - [8.1. nicht — Negating Verbs, Adjectives & Whole Clauses](#81-nicht-negating-verbs-adjectives-whole-clauses)
  - [8.2. kein/keine — Negating Nouns](#82-keinkeine-negating-nouns)
  - [8.3. nicht vs. kein Decision Rule](#83-nicht-vs-kein-decision-rule)
- [9. es gibt & Impersonal es](#9-es-gibt-impersonal-es)
  - [9.1. es gibt + Accusative](#91-es-gibt-accusative)
  - [9.2. Weather Expressions](#92-weather-expressions)
  - [9.3. Fixed Impersonal Phrases](#93-fixed-impersonal-phrases)
- [10. Practice Exercises](#10-practice-exercises)
  - [10.1. Pronunciation Drill](#101-pronunciation-drill)
  - [10.2. Conjugation Drill](#102-conjugation-drill)
  - [10.3. Case Identification](#103-case-identification)
  - [10.4. Word Order Drill](#104-word-order-drill)
  - [10.5. Question Formation](#105-question-formation)
  - [10.6. Negation Transformation](#106-negation-transformation)
  - [10.7. Translation Exercises](#107-translation-exercises)
  - [10.8. Answer Key](#108-answer-key)

<a id="1-german-pronunciation-guide"></a>

## 1. German Pronunciation Guide

<a id="11-vowels-short-long"></a>

### 1.1. Vowels — Short & Long

Every German vowel has a **short** and a **long** form. Long vowels are tense and held longer; short vowels are lax and clipped. A doubled vowel (aa, ee, oo) or a vowel followed by **h** signals length. A vowel followed by a double consonant is short.

| Letter | Long | IPA | Example | Short | IPA | Example |
|--------|------|-----|---------|-------|-----|---------|
| a | "ah" in *father* | [aː] | Vater | "a" in *cut* | [a] | Mann |
| e | "ay" in *say* | [eː] | gehen | "e" in *bed* | [ɛ] | Bett |
| i | "ee" in *see* | [iː] | Liebe | "i" in *bit* | [ɪ] | Tisch |
| o | "oh" in *go* | [oː] | Sohn | "o" in *lot* | [ɔ] | Sonne |
| u | "oo" in *food* | [uː] | Buch | "u" in *put* | [ʊ] | Hund |
| ä | "ai" in *hair* | [ɛː] | Mädchen | "e" in *bed* | [ɛ] | Männer |
| ö | "eu" in French *peur* | [øː] | schön | "i" in *bird* (rounded) | [œ] | Wörter |
| ü | "u" in French *tu* | [yː] | über | French *tu* (short) | [ʏ] | müssen |

Unstressed **e** at the end of a word or in prefixes (ge-, be-) reduces to schwa [ə]: *Blume* [ˈbluːmə], *gesehen* [ɡəˈzeːən].

<a id="12-consonants-final-devoicing"></a>

### 1.2. Consonants & Final Devoicing

Most consonants sound like English. The key difference is **Auslautverhärtung** (final devoicing): voiced consonants become voiceless at the end of a word or syllable.

| Voiced | Devoiced | Example | Pronunciation |
|--------|----------|---------|---------------|
| b [b] | → p [p] | Urlaub | [ˈuːɐ̯laʊ̯p] |
| d [d] | → t [t] | Hund | [hʊnt] |
| g [ɡ] | → k [k] | Tag | [taːk] |

Compare: **Hunde** [ˈhʊndə] (voiced — d before vowel) vs. **Hund** [hʊnt] (devoiced — d at end).

<a id="13-special-combinations"></a>

### 1.3. Special Combinations

| Spelling | Sound | IPA | Example |
|----------|-------|-----|---------|
| ch (after a, o, u, au) | back of throat, like Scottish *loch* | [x] | Bach, Buch, auch |
| ch (after e, i, ä, ö, ü, ei, eu) | soft, like *h* in *huge* | [ç] | ich, Milch, Bücher |
| sch | "sh" in *ship* | [ʃ] | Schule, schön |
| sp (word-initial) | "shp" | [ʃp] | spielen, sprechen |
| st (word-initial) | "sht" | [ʃt] | stehen, Straße |
| v | "f" | [f] | Vater, viel |
| w | "v" | [v] | Wasser, wir |
| z | "ts" in *cats* | [ts] | Zeit, zehn |
| ß (Eszett) | always voiceless "s" (after long vowel/diphthong) | [s] | groß, Straße, heiß |
| j | "y" in *yes* | [j] | ja, jung |
| r (before vowels) | uvular | [ʁ] | rot, Bruder |
| r (after vowels / syllable-final) | vocalized | [ɐ] | Vater, wir |

<a id="14-diphthongs"></a>

### 1.4. Diphthongs

| Spelling | Sound | IPA | Example |
|----------|-------|-----|---------|
| au | "ow" in *cow* | [aʊ̯] | Baum, Haus |
| ei, ai | "i" in *mine* | [aɪ̯] | mein, eins |
| eu, äu | "oy" in *boy* | [ɔʏ̯] | neu, Bäume |
| ie | long "ee" (not a true diphthong) | [iː] | Liebe, spielen |

<a id="2-essential-vocabulary-150-words"></a>

## 2. Essential Vocabulary (150 Words)

<a id="21-nouns-40-with-gender-derdiedas-plural"></a>

### 2.1. Nouns (40) — with Gender (der/die/das) & Plural

Every German noun has a grammatical gender (masculine, feminine, or neuter) and must be learned with its article. Plural forms are shown in parentheses.

##### Family & People
1. der Vater (Väter) — father
2. die Mutter (Mütter) — mother
3. der Bruder (Brüder) — brother
4. die Schwester (-n) — sister
5. das Kind (-er) — child
6. der Mann (Männer) — man, husband
7. die Frau (-en) — woman, wife
8. der Freund (-e) — friend (male)
9. die Freundin (-nen) — friend (female)
10. das Baby (-s) — baby

##### Food & Drink
11. das Brot (-e) — bread
12. die Milch (no pl.) — milk
13. das Wasser (—) — water
14. der Kaffee (-s) — coffee
15. der Tee (-s) — tea
16. der Apfel (Äpfel) — apple
17. das Ei (-er) — egg
18. das Fleisch (no pl.) — meat
19. der Käse (—) — cheese
20. die Suppe (-n) — soup

##### Home & Objects
21. das Haus (Häuser) — house
22. die Tür (-en) — door
23. das Fenster (—) — window
24. der Tisch (-e) — table
25. der Stuhl (Stühle) — chair
26. das Bett (-en) — bed
27. die Lampe (-n) — lamp
28. das Buch (Bücher) — book
29. die Uhr (-en) — clock, watch
30. der Schlüssel (—) — key

##### Places & Nature
31. die Schule (-n) — school
32. die Stadt (Städte) — city
33. der Park (-s) — park
34. die Straße (-n) — street
35. der Baum (Bäume) — tree
36. die Blume (-n) — flower
37. der Berg (-e) — mountain
38. der See (-n) — lake
39. die Sonne (-n) — sun
40. der Hund (-e) — dog

<a id="22-verbs-40"></a>

### 2.2. Verbs (40)

Verbs are listed in the infinitive form. **sein** (to be) and **haben** (to have) — the two most fundamental verbs — are covered with full conjugation charts in Sections 4.2–4.3. Irregular conjugation patterns for other verbs are covered in Section 4.5.

##### Daily Routine
1. aufstehen — to get up
2. essen — to eat
3. schlafen — to sleep
4. trinken — to drink
5. wohnen — to live, reside
6. arbeiten — to work
7. spielen — to play
8. machen — to do, make
9. kaufen — to buy
10. brauchen — to need

##### Communication
11. sprechen — to speak
12. fragen — to ask
13. antworten — to answer
14. sagen — to say
15. hören — to hear
16. sehen — to see
17. schreiben — to write
18. lesen — to read
19. geben — to give
20. helfen — to help

##### Movement
21. gehen — to go, walk
22. kommen — to come
23. laufen — to run, walk
24. fahren — to drive, travel
25. fliegen — to fly
26. schwimmen — to swim
27. stehen — to stand
28. sitzen — to sit
29. liegen — to lie (position)
30. nehmen — to take

##### Cognitive
31. denken — to think
32. wissen — to know (fact)
33. glauben — to believe
34. kennen — to know (person/place)
35. lernen — to learn
36. vergessen — to forget
37. verstehen — to understand
38. meinen — to mean, think
39. finden — to find
40. hoffen — to hope

Note: *aufstehen* (#1) is a **separable verb** — its prefix splits off in conjugation (*ich stehe auf*, not ~~ich aufstehe~~). Separable verbs are fully explained in Week 5; for now, learn it as vocabulary.

<a id="23-adjectives-35"></a>

### 2.3. Adjectives (35)

##### Description (10)
1. groß — big, tall
2. klein — small, little
3. alt — old
4. neu — new
5. lang — long
6. kurz — short
7. schön — beautiful, nice
8. hässlich — ugly
9. heiß — hot
10. kalt — cold

##### Emotion & State (10)
11. gut — good
12. schlecht — bad
13. müde — tired
14. hungrig — hungry
15. durstig — thirsty
16. froh — glad, happy
17. traurig — sad
18. krank — sick
19. gesund — healthy
20. glücklich — happy, lucky

##### Size & Quantity (8)
21. viel — much, a lot
22. wenig — little, few
23. mehr — more
24. weniger — less, fewer
25. ganz — whole, entirely
26. genug — enough
27. leer — empty
28. voll — full

##### Quality (7)
29. schnell — fast
30. langsam — slow
31. schwer — heavy, difficult
32. leicht — light, easy
33. laut — loud
34. leise — quiet
35. stark — strong

<a id="24-adverbs-of-time-frequency-15"></a>

### 2.4. Adverbs of Time & Frequency (15)

| # | German | English |
|---|--------|---------|
| 1 | heute | today |
| 2 | morgen | tomorrow |
| 3 | gestern | yesterday |
| 4 | jetzt | now |
| 5 | immer | always |
| 6 | oft | often |
| 7 | manchmal | sometimes |
| 8 | selten | rarely |
| 9 | nie | never |
| 10 | bald | soon |
| 11 | gerade | right now, just |
| 12 | schon | already |
| 13 | noch | still, yet |
| 14 | wieder | again |
| 15 | zuerst | first, at first |

<a id="25-adverbs-of-place-manner-10"></a>

### 2.5. Adverbs of Place & Manner (10)

| # | German | English |
|---|--------|---------|
| 1 | hier | here |
| 2 | dort | there |
| 3 | oben | above, upstairs |
| 4 | unten | below, downstairs |
| 5 | links | left |
| 6 | rechts | right |
| 7 | sehr | very |
| 8 | zu | too (excessive) |
| 9 | auch | also, too |
| 10 | gern | gladly, with pleasure |

<a id="26-common-phrases-particles-10"></a>

### 2.6. Common Phrases & Particles (10)

| # | German | English |
|---|--------|---------|
| 1 | ja | yes |
| 2 | nein | no |
| 3 | bitte | please |
| 4 | danke | thank you |
| 5 | bitte schön | you're welcome |
| 6 | Entschuldigung | excuse me, sorry |
| 7 | und | and |
| 8 | oder | or |
| 9 | aber | but |
| 10 | nicht | not |

<a id="3-numbers-calendar"></a>

## 3. Numbers & Calendar

<a id="31-cardinal-numbers-0-100"></a>

### 3.1. Cardinal Numbers 0–100

##### 0–12 (memorize individually)

| # | German | # | German | # | German |
|---|--------|---|--------|---|--------|
| 0 | null | 5 | fünf | 10 | zehn |
| 1 | eins | 6 | sechs | 11 | elf |
| 2 | zwei | 7 | sieben | 12 | zwölf |
| 3 | drei | 8 | acht | | |
| 4 | vier | 9 | neun | | |

##### 13–19 (ones + zehn)

dreizehn (13), vierzehn (14), fünfzehn (15), **sechzehn** (16, not ~~sechszehn~~), **siebzehn** (17, not ~~siebenzehn~~), achtzehn (18), neunzehn (19)

##### Tens 20–100

zwanzig (20), **dreißig** (30, not ~~dreizig~~ — the only tens form with -ßig), vierzig (40), fünfzig (50), **sechzig** (60, not ~~sechszig~~), **siebzig** (70, not ~~siebenzig~~), achtzig (80), neunzig (90), (ein)hundert (100)

##### 21–99 pattern: ones + und + tens

Read right to left: **einundzwanzig** (21, lit. "one-and-twenty"), zweiunddreißig (32), dreiundvierzig (43), vierundfünfzig (54), fünfundsechzig (65), sechsundsiebzig (76), siebenundachtzig (87), achtundneunzig (98).

<a id="32-cardinal-numbers-100-1000000"></a>

### 3.2. Cardinal Numbers 100–1,000,000

Numbers above 100 are built by compounding — written as **one word** (no spaces) up to 999,999:

| Number | German |
|--------|--------|
| 100 | (ein)hundert |
| 200 | zweihundert |
| 432 | vierhundertzweiunddreißig |
| 1,000 | (ein)tausend |
| 3,500 | dreitausendfünfhundert |
| 10,000 | zehntausend |
| 1,000,000 | eine Million (feminine noun, separate word) |
| 2,000,000 | zwei Millionen |
| 1,000,000,000 | eine Milliarde |

<a id="33-ordinal-numbers"></a>

### 3.3. Ordinal Numbers

Ordinals are formed by adding **-te** (1st–19th) or **-ste** (20th+) to the cardinal stem, plus an adjective ending. Three irregulars: **erste** (1st), **dritte** (3rd), **achte** (8th, not ~~achtte~~).

| Ordinal | German | With *am* (on the...) |
|---------|--------|-----------------------|
| 1st | erste | am ersten |
| 2nd | zweite | am zweiten |
| 3rd | dritte | am dritten |
| 4th | vierte | am vierten |
| 7th | sieb(en)te | am sieb(en)ten |
| 8th | achte | am achten |
| 20th | zwanzigste | am zwanzigsten |
| 100th | hundertste | am hundertsten |

<a id="34-days-of-the-week"></a>

### 3.4. Days of the Week

All days are **masculine** (der). Use **am** + day for "on [day]."

| German | English | Usage |
|--------|---------|-------|
| der Montag | Monday | am Montag |
| der Dienstag | Tuesday | am Dienstag |
| der Mittwoch | Wednesday | am Mittwoch |
| der Donnerstag | Thursday | am Donnerstag |
| der Freitag | Friday | am Freitag |
| der Samstag | Saturday | am Samstag |
| der Sonntag | Sunday | am Sonntag |

<a id="35-months"></a>

### 3.5. Months

All months are **masculine** (der). Use **im** + month for "in [month]."

der Januar, der Februar, der März, der April, der Mai, der Juni, der Juli, der August, der September, der Oktober, der November, der Dezember

*Im März beginnt der Frühling.* (In March, spring begins.)

<a id="36-seasons"></a>

### 3.6. Seasons

All seasons are **masculine** (der). Use **im** + season for "in [season]."

| German | English |
|--------|---------|
| der Frühling | spring |
| der Sommer | summer |
| der Herbst | autumn |
| der Winter | winter |

*Im Sommer ist es heiß.* (In summer it is hot.)

<a id="4-personal-pronouns-present-tense-präsens"></a>

## 4. Personal Pronouns & Present Tense (Präsens)

<a id="41-personal-pronouns"></a>

### 4.1. Personal Pronouns

| German | English | Notes |
|--------|---------|-------|
| ich | I | |
| du | you | informal singular |
| er | he | |
| sie | she | |
| es | it | |
| wir | we | |
| ihr | you | informal plural |
| sie | they | same form as "she" — context distinguishes |
| Sie | you | formal (always capitalized), singular or plural |

<a id="42-conjugation-of-sein"></a>

### 4.2. Conjugation of sein

*sein* (to be) — highly irregular, must be memorized.

| Person | Conjugation |
|--------|-------------|
| ich | bin |
| du | bist |
| er/sie/es | ist |
| wir | sind |
| ihr | seid |
| sie/Sie | sind |

*Ich bin müde.* (I am tired.) — *Wir sind Freunde.* (We are friends.)

<a id="43-conjugation-of-haben"></a>

### 4.3. Conjugation of haben

*haben* (to have) — irregular in du and er/sie/es forms.

| Person | Conjugation |
|--------|-------------|
| ich | habe |
| du | hast |
| er/sie/es | hat |
| wir | haben |
| ihr | habt |
| sie/Sie | haben |

*Er hat ein Buch.* (He has a book.) — *Habt ihr Zeit?* (Do you have time?)

<a id="44-regular-weak-verb-pattern"></a>

### 4.4. Regular (Weak) Verb Pattern

For regular verbs, remove **-en** from the infinitive to get the stem, then add the endings:

| Person | Ending | spielen (play) | machen (do) | lernen (learn) |
|--------|--------|----------------|-------------|----------------|
| ich | -e | spiele | mache | lerne |
| du | -st | spielst | machst | lernst |
| er/sie/es | -t | spielt | macht | lernt |
| wir | -en | spielen | machen | lernen |
| ihr | -t | spielt | macht | lernt |
| sie/Sie | -en | spielen | machen | lernen |

If the stem ends in **-t** or **-d**, add an **-e-** before -st and -t for pronunciation:

| Person | arbeiten (work) | finden (find) |
|--------|-----------------|---------------|
| du | arbeit**e**st | find**e**st |
| er/sie/es | arbeit**e**t | find**e**t |
| ihr | arbeit**e**t | find**e**t |

<a id="45-key-irregular-verbs-in-präsens-with-vowel-changes"></a>

### 4.5. Key Irregular Verbs in Präsens with Vowel Changes

Seven high-frequency verbs change their stem vowel in the **du** and **er/sie/es** forms only. The wir/ihr/sie forms keep the base vowel.

| Person | sehen (e→ie) | essen (e→i) | sprechen (e→i) | laufen (au→äu) | nehmen (e→i) | geben (e→i) | helfen (e→i) |
|--------|-------------|------------|---------------|---------------|-------------|------------|-------------|
| ich | sehe | esse | spreche | laufe | nehme | gebe | helfe |
| du | **siehst** | **isst** | **sprichst** | **läufst** | **nimmst** | **gibst** | **hilfst** |
| er/sie/es | **sieht** | **isst** | **spricht** | **läuft** | **nimmt** | **gibt** | **hilft** |
| wir | sehen | essen | sprechen | laufen | nehmen | geben | helfen |
| ihr | seht | esst | sprecht | lauft | nehmt | gebt | helft |
| sie/Sie | sehen | essen | sprechen | laufen | nehmen | geben | helfen |

Note: *nehmen* also changes its consonant (h drops, m doubles: du n**imm**st, er n**imm**t).

Other vocabulary verbs with vowel changes: **fahren** (a→ä: du f**ä**hrst, er f**ä**hrt), **schlafen** (a→ä: du schl**ä**fst, er schl**ä**ft), **lesen** (e→ie: du l**ie**st, er l**ie**st), **fallen** (a→ä: du f**ä**llst, er f**ä**llt). Also fully irregular: **wissen** (ich **weiß**, du **weißt**, er **weiß**, wir wissen).

<a id="5-nominative-case-the-subject"></a>

## 5. Nominative Case — The Subject

The **nominative case** marks the subject of the sentence — the person or thing performing the action.

<a id="51-definite-articles"></a>

### 5.1. Definite Articles

| Gender | Nominative | Example |
|--------|------------|---------|
| Masculine | **der** | der Mann (the man) |
| Feminine | **die** | die Frau (the woman) |
| Neuter | **das** | das Kind (the child) |
| Plural (all) | **die** | die Kinder (the children) |

<a id="52-indefinite-articles"></a>

### 5.2. Indefinite Articles

| Gender | Nominative | Example |
|--------|------------|---------|
| Masculine | **ein** | ein Mann (a man) |
| Feminine | **eine** | eine Frau (a woman) |
| Neuter | **ein** | ein Kind (a child) |
| Plural | — | Kinder (children) — no indefinite plural |

<a id="53-identifying-the-subject-wer-was"></a>

### 5.3. Identifying the Subject: "wer?" / "was?"

To find the nominative, ask:
- **Wer?** (Who?) — for people: *Wer spielt?* → **Der Mann** spielt.
- **Was?** (What?) — for things: *Was ist gut?* → **Das Buch** ist gut.

The answer to *wer?* or *was?* is always in the nominative case.

<a id="6-accusative-case-direct-objects"></a>

## 6. Accusative Case — Direct Objects

The **accusative case** marks the direct object — the person or thing directly receiving the action of a transitive verb.

<a id="61-accusative-articles"></a>

### 6.1. Accusative Articles

| Gender | Nominative | Accusative | Change? |
|--------|------------|------------|---------|
| Masculine | der / ein | **den** / **einen** | YES — only masculine changes |
| Feminine | die / eine | die / eine | no change |
| Neuter | das / ein | das / ein | no change |
| Plural | die | die | no change |

**Key rule:** Only the masculine article changes in the accusative. Everything else stays the same.

<a id="62-transitive-verbs-that-take-accusative"></a>

### 6.2. Transitive Verbs That Take Accusative

These 10 verbs always take a direct object in the accusative:

| Verb | Example |
|------|---------|
| haben (to have) | Ich habe **einen Hund**. |
| sehen (to see) | Er sieht **den Mann**. |
| essen (to eat) | Sie isst **einen Apfel**. |
| trinken (to drink) | Wir trinken **den Kaffee**. |
| kaufen (to buy) | Er kauft **ein Buch**. |
| lesen (to read) | Ich lese **das Buch**. |
| finden (to find) | Sie findet **den Schlüssel**. |
| nehmen (to take) | Er nimmt **den Stuhl**. |
| brauchen (to need) | Wir brauchen **ein Bett**. |
| kennen (to know) | Ich kenne **die Frau**. |

<a id="63-recognizing-direct-objects-wen-was"></a>

### 6.3. Recognizing Direct Objects: "wen?" / "was?"

To find the accusative, ask:
- **Wen?** (Whom?) — for people: *Ich sehe* **wen?** → Ich sehe **den Mann**.
- **Was?** (What?) — for things: *Er kauft* **was?** → Er kauft **ein Buch**.

<a id="7-word-order-the-v2-rule-questions"></a>

## 7. Word Order — The V2 Rule & Questions

<a id="71-verb-second-in-main-clauses"></a>

### 7.1. Verb-Second in Main Clauses

In every German main clause, the conjugated verb must occupy **position 2**. This is the V2 rule.

An **"element"** is a complete grammatical chunk — not just a single word. A time phrase like *heute Morgen* counts as one element. A subject like *mein Bruder* counts as one element.

Formula: **[Element 1] + [VERB] + [remaining elements]**

- Das Kind **spielt** Fußball. (Position 1 = subject)
- Heute **spielt** das Kind Fußball. (Position 1 = time adverb)

The verb is always second, regardless of what fills position 1.

<a id="72-subject-verb-inversion-when-position-1-subject"></a>

### 7.2. Subject-Verb Inversion When Position 1 ≠ Subject

When something other than the subject occupies position 1, the subject moves to position 3 (immediately after the verb). This is called **inversion**.

| Position 1 | Verb (Pos 2) | Subject (Pos 3) | Rest |
|------------|-------------|-----------------|------|
| **Heute** | lerne | ich | Deutsch. |
| **Dort** | wohnt | mein Bruder. | |
| **Leider** | hat | er | kein Buch. |

1. **Heute** lerne ich Deutsch. (Today I learn German.)
2. **Dort** wohnt mein Bruder. (There lives my brother.)
3. **Leider** hat er kein Buch. (Unfortunately he has no book.)

<a id="73-yesno-questions-v1"></a>

### 7.3. Yes/No Questions — V1

For yes/no questions, move the verb to **position 1** (V1). The subject follows directly.

| Statement | Question |
|-----------|----------|
| Du spielst Fußball. | **Spielst** du Fußball? |
| Er ist müde. | **Ist** er müde? |
| Sie hat ein Buch. | **Hat** sie ein Buch? |

<a id="74-w-questions"></a>

### 7.4. W-Questions

Place the W-word in position 1 and the verb in position 2 (still V2). The subject follows.

| W-Word | Meaning | Example |
|--------|---------|---------|
| wer | who | **Wer** ist das? (Who is that?) |
| was | what | **Was** machst du? (What do you do?) |
| wo | where | **Wo** wohnst du? (Where do you live?) |
| wann | when | **Wann** kommst du? (When do you come?) |
| warum | why | **Warum** lernst du Deutsch? (Why do you learn German?) |
| wie | how | **Wie** geht es dir? (How are you?) |
| woher | where from | **Woher** kommst du? (Where do you come from?) |
| wohin | where to | **Wohin** gehst du? (Where are you going?) |

<a id="8-negation"></a>

## 8. Negation

<a id="81-nicht-negating-verbs-adjectives-whole-clauses"></a>

### 8.1. nicht — Negating Verbs, Adjectives & Whole Clauses

**nicht** negates verbs, adjectives, adverbs, and specific (definite) nouns. Placement rules:

| What is negated | Placement | Example |
|----------------|-----------|---------|
| Predicate adjective | before the adjective | Das Buch ist **nicht** gut. |
| Prepositional phrase | before the phrase | Er geht **nicht** in die Schule. |
| Whole verb / clause | at the end | Ich spiele **nicht**. |
| Specific element | directly before it | Er kommt **nicht** heute (, sondern morgen). |

<a id="82-keinkeine-negating-nouns"></a>

### 8.2. kein/keine — Negating Nouns

**kein** negates nouns. It replaces **ein/eine** or a bare noun (without article). It follows the same endings as *ein*:

| Case | Masculine | Feminine | Neuter | Plural |
|------|-----------|----------|--------|--------|
| Nominative | kein | keine | kein | keine |
| Accusative | keinen | keine | kein | keine |

- Ich habe **ein** Buch → Ich habe **kein** Buch. (I have no book.)
- Er trinkt Milch → Er trinkt **keine** Milch. (He drinks no milk.)

<a id="83-nicht-vs-kein-decision-rule"></a>

### 8.3. nicht vs. kein Decision Rule

| Situation | Use | Example |
|-----------|-----|---------|
| Noun with ein/eine | **kein** | ein Buch → **kein** Buch |
| Bare noun (no article) | **kein** | Milch → **keine** Milch |
| Verb, adjective, adverb, or noun with der/die/das | **nicht** | gut → **nicht** gut |

**4 contrastive pairs:**

| Affirmative | Negative | Rule |
|------------|----------|------|
| Ich habe **ein** Buch. | Ich habe **kein** Buch. | ein → kein |
| Er trinkt **Wasser**. | Er trinkt **kein** Wasser. | bare noun → kein |
| Das Buch ist **gut**. | Das Buch ist **nicht** gut. | adjective → nicht |
| Er kommt **heute**. | Er kommt heute **nicht**. | verb/adverb → nicht |

<a id="9-es-gibt-impersonal-es"></a>

## 9. es gibt & Impersonal es

<a id="91-es-gibt-accusative"></a>

### 9.1. es gibt + Accusative

**Es gibt** means "there is" / "there are." The noun following it is always in the **accusative** case.

1. Es gibt **einen Park** hier. (There is a park here.)
2. Es gibt **viele Bäume**. (There are many trees.)
3. Es gibt **keinen Kaffee** mehr. (There is no more coffee.)
4. **Gibt es** ein Fenster? (Is there a window?)
5. Es gibt heute **Suppe**. (There is soup today.)

To form a question, invert: *Gibt es...?*

<a id="92-weather-expressions"></a>

### 9.2. Weather Expressions

Weather uses impersonal **es** as the subject. These are fixed patterns.

- Es **regnet**. (It's raining.)
- Es **schneit**. (It's snowing.)
- Es **ist** kalt. (It's cold.)
- Es **ist** warm. (It's warm.)
- Es **ist** sonnig. (It's sunny.)

<a id="93-fixed-impersonal-phrases"></a>

### 9.3. Fixed Impersonal Phrases

These common expressions use impersonal **es**. Learn them as set phrases.

| German | Literal | Meaning |
|--------|---------|---------|
| Es geht mir gut. | It goes me well. | I'm doing well. |
| Es geht mir schlecht. | It goes me badly. | I'm doing badly. |
| Es tut mir leid. | It does me sorrow. | I'm sorry. |
| Es freut mich. | It pleases me. | I'm pleased. / Nice to meet you. |

<a id="10-practice-exercises"></a>

## 10. Practice Exercises

<a id="101-pronunciation-drill"></a>

### 10.1. Pronunciation Drill

Read each sentence aloud, focusing on vowel length, final devoicing, and ch sounds. IPA transcription is provided.

1. Der Hund läuft schnell. [deːɐ̯ hʊnt lɔʏ̯ft ʃnɛl]
2. Ich spreche Deutsch. [ɪç ʃpʁɛçə dɔʏ̯tʃ]
3. Das Buch ist sehr gut. [das buːx ɪst zeːɐ̯ ɡuːt]
4. Sie trinkt kalte Milch. [ziː tʁɪŋkt kaltə mɪlç]
5. Der Vater arbeitet viel. [deːɐ̯ faːtɐ ˈaʁbaɪ̯tət fiːl]
6. Wir gehen in die Schule. [viːɐ̯ ˈɡeːən ɪn diː ˈʃuːlə]
7. Der Baum ist groß und schön. [deːɐ̯ baʊ̯m ɪst ɡʁoːs ʊnt ʃøːn]
8. Die Blume ist schön. [diː ˈbluːmə ɪst ʃøːn]
9. Ich bin müde und hungrig. [ɪç bɪn ˈmyːdə ʊnt ˈhʊŋʁɪç]
10. Das Kind schläft im Bett. [das kɪnt ʃlɛːft ɪm bɛt]

<a id="102-conjugation-drill"></a>

### 10.2. Conjugation Drill

Fill in the correct Präsens form of the verb in parentheses.

1. Ich ___ (sein) Student.
2. Du ___ (haben) ein Buch.
3. Er ___ (spielen) Fußball.
4. Wir ___ (lernen) Deutsch.
5. Ihr ___ (machen) das gut.
6. Sie (pl.) ___ (arbeiten) viel.
7. Ich ___ (sehen) den Baum.
8. Du ___ (essen) einen Apfel.
9. Er ___ (sprechen) Deutsch.
10. Sie (sg.) ___ (laufen) schnell.
11. Wir ___ (sein) müde.
12. Du ___ (nehmen) das Buch.
13. Es ___ (geben) hier einen Park.
14. Du ___ (helfen) gern.
15. Ihr ___ (haben) einen Hund.

<a id="103-case-identification"></a>

### 10.3. Case Identification

The nominative form of the article is given in parentheses. Write the correct form for each position and label each noun phrase as Nominative (Nom) or Accusative (Acc). Remember: only masculine articles change in the accusative (der→den, ein→einen).

1. ___ (der) Mann sieht ___ (das) Kind.
2. ___ (die) Frau kauft ___ (ein) Apfel.
3. ___ (das) Kind trinkt ___ (die) Suppe.
4. ___ (der) Freund liest ___ (ein) Buch.
5. ___ (die) Mutter findet ___ (der) Schlüssel.
6. ___ (der) Bruder nimmt ___ (der) Stuhl.
7. ___ (die) Schwester hat ___ (ein) Hund.
8. ___ (der) Vater kennt ___ (die) Frau.
9. ___ (ein) Kind braucht ___ (ein) Bett.
10. ___ (der) Mann kauft ___ (ein) Ei.
11. ___ (eine) Freundin sieht ___ (der) Baum.
12. ___ (die) Frau nimmt ___ (das) Brot.

<a id="104-word-order-drill"></a>

### 10.4. Word Order Drill

Rearrange the given elements into a correct V2 sentence. Items 5–8 must place the time/frequency word first (inversion required).

1. Fußball / spielt / das Kind
2. ist / das Buch / gut
3. die Frau / den Apfel / isst
4. trinkt / der Vater / Kaffee
5. heute / lerne / ich / Deutsch
6. morgen / er / kommt
7. jetzt / müde / du / bist
8. oft / sie / spielt / hier

<a id="105-question-formation"></a>

### 10.5. Question Formation

**A. Yes/No Questions** — Convert these statements into yes/no questions.

1. Er spielt Fußball.
2. Sie hat ein Buch.
3. Du bist müde.
4. Wir gehen in die Schule.
5. Das Kind trinkt Milch.

**B. W-Questions** — Form a W-question using the word in parentheses.

1. Er wohnt in Berlin. (wo)
2. Sie kommt morgen. (wann)
3. Der Mann kauft ein Buch. (was)
4. Sie lernt Deutsch. (warum)
5. Er kommt aus Berlin. (woher)

<a id="106-negation-transformation"></a>

### 10.6. Negation Transformation

Make each sentence negative using **nicht** or **kein/keine/keinen**.

1. Ich habe ein Buch.
2. Er spielt gut.
3. Sie trinkt Milch.
4. Wir haben einen Hund.
5. Das Buch ist gut.
6. Er geht in die Schule.
7. Ich habe Zeit.
8. Der Mann ist groß.
9. Sie hat einen Freund.
10. Wir spielen heute.

<a id="107-translation-exercises"></a>

### 10.7. Translation Exercises

Translate each sentence into German using only Week 1 grammar and vocabulary.

1. The man sees the child.
2. I am tired.
3. She eats an apple.
4. We learn German.
5. The woman has a book.
6. He runs fast.
7. There is a park here.
8. It is cold today.
9. The child has no dog.
10. Where does she work?
11. Is the book good?
12. I am not hungry.
13. The mother reads a book.
14. He drinks no coffee.
15. When does he come?

<a id="108-answer-key"></a>

### 10.8. Answer Key

##### 10.2. Conjugation Drill

1. Ich **bin** Student. (I am a student.)
2. Du **hast** ein Buch. (You have a book.)
3. Er **spielt** Fußball. (He plays soccer.)
4. Wir **lernen** Deutsch. (We learn German.)
5. Ihr **macht** das gut. (You all do that well.)
6. Sie **arbeiten** viel. (They work a lot.)
7. Ich **sehe** den Baum. (I see the tree.)
8. Du **isst** einen Apfel. (You eat an apple.)
9. Er **spricht** Deutsch. (He speaks German.)
10. Sie **läuft** schnell. (She runs fast.)
11. Wir **sind** müde. (We are tired.)
12. Du **nimmst** das Buch. (You take the book.)
13. Es **gibt** hier einen Park. (There is [lit. it gives] here a park.)
14. Du **hilfst** gern. (You help gladly.)
15. Ihr **habt** einen Hund. (You all have a dog.)

##### 10.3. Case Identification

1. **Der** Mann (Nom) sieht **das** Kind (Acc). (The man sees the child.)
2. **Die** Frau (Nom) kauft **einen** Apfel (Acc). (The woman buys an apple.)
3. **Das** Kind (Nom) trinkt **die** Suppe (Acc). (The child drinks the soup.)
4. **Der** Freund (Nom) liest **ein** Buch (Acc). (The friend reads a book.)
5. **Die** Mutter (Nom) findet **den** Schlüssel (Acc). (The mother finds the key.)
6. **Der** Bruder (Nom) nimmt **den** Stuhl (Acc). (The brother takes the chair.)
7. **Die** Schwester (Nom) hat **einen** Hund (Acc). (The sister has a dog.)
8. **Der** Vater (Nom) kennt **die** Frau (Acc). (The father knows the woman.)
9. **Ein** Kind (Nom) braucht **ein** Bett (Acc). (A child needs a bed.)
10. **Der** Mann (Nom) kauft **ein** Ei (Acc). (The man buys an egg.)
11. **Eine** Freundin (Nom) sieht **den** Baum (Acc). (A friend sees the tree.)
12. **Die** Frau (Nom) nimmt **das** Brot (Acc). (The woman takes the bread.)

##### 10.4. Word Order Drill

1. Das Kind spielt Fußball. (The child plays soccer.)
2. Das Buch ist gut. (The book is good.)
3. Die Frau isst den Apfel. (The woman eats the apple.)
4. Der Vater trinkt Kaffee. (The father drinks coffee.)
5. Heute lerne ich Deutsch. (Today learn I German.)
6. Morgen kommt er. (Tomorrow comes he.)
7. Jetzt bist du müde. (Now are you tired.)
8. Oft spielt sie hier. (Often plays she here.)

##### 10.5. Question Formation

**A. Yes/No Questions:**
1. Spielt er Fußball? (Plays he soccer?)
2. Hat sie ein Buch? (Has she a book?)
3. Bist du müde? (Are you tired?)
4. Gehen wir in die Schule? (Go we into the school?)
5. Trinkt das Kind Milch? (Drinks the child milk?)

**B. W-Questions:**
1. Wo wohnt er? (Where lives he?)
2. Wann kommt sie? (When comes she?)
3. Was kauft der Mann? (What buys the man?)
4. Warum lernt sie Deutsch? (Why learns she German?)
5. Woher kommt er? (Where-from comes he?)

##### 10.6. Negation Transformation

1. Ich habe **kein** Buch. (I have no book.) — *ein → kein*
2. Er spielt **nicht** gut. (He plays not well.) — *adverb → nicht*
3. Sie trinkt **keine** Milch. (She drinks no milk.) — *bare noun → kein*
4. Wir haben **keinen** Hund. (We have no dog.) — *einen → keinen*
5. Das Buch ist **nicht** gut. (The book is not good.) — *adjective → nicht*
6. Er geht **nicht** in die Schule. (He goes not into the school.) — *prep. phrase → nicht*
7. Ich habe **keine** Zeit. (I have no time.) — *bare noun → kein*
8. Der Mann ist **nicht** groß. (The man is not tall.) — *adjective → nicht*
9. Sie hat **keinen** Freund. (She has no friend.) — *einen → keinen*
10. Wir spielen heute **nicht**. (We play today not.) — *verb → nicht at end*

##### 10.7. Translation Exercises

1. **Der Mann sieht das Kind.** (The man sees the child.)
2. **Ich bin müde.** (I am tired.)
3. **Sie isst einen Apfel.** (She eats an apple.)
4. **Wir lernen Deutsch.** (We learn German.)
5. **Die Frau hat ein Buch.** (The woman has a book.)
6. **Er läuft schnell.** (He runs fast.)
7. **Es gibt hier einen Park.** (It gives here a park.)
8. **Es ist heute kalt.** (It is today cold.)
9. **Das Kind hat keinen Hund.** (The child has no dog.)
10. **Wo arbeitet sie?** (Where works she?)
11. **Ist das Buch gut?** (Is the book good?)
12. **Ich bin nicht hungrig.** (I am not hungry.)
13. **Die Mutter liest ein Buch.** (The mother reads a book.)
14. **Er trinkt keinen Kaffee.** (He drinks no coffee.)
15. **Wann kommt er?** (When comes he?)
